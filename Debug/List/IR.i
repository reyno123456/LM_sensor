#line 1 "D:\\avr\\Uart_Interrupt\\LM_Universal\\LM_IAR_V5.0\\Alan\\IR.c"
...
#line 14 "D:\\avr\\Uart_Interrupt\\LM_Universal\\LM_IAR_V5.0\\Alan\\IR.c"
...
#line 15 "D:\\avr\\Uart_Interrupt\\LM_Universal\\LM_IAR_V5.0\\Alan\\IR.c"
...
#line 16 "D:\\avr\\Uart_Interrupt\\LM_Universal\\LM_IAR_V5.0\\Alan\\IR.c"
...
#line 17 "D:\\avr\\Uart_Interrupt\\LM_Universal\\LM_IAR_V5.0\\Alan\\IR.c"
...
/****************	add by alan end	**************************/
#line 18 "D:\\avr\\Uart_Interrupt\\LM_Universal\\LM_IAR_V5.0\\Alan\\IR.c"
...
// add by alan end
#line 19 "D:\\avr\\Uart_Interrupt\\LM_Universal\\LM_IAR_V5.0\\Alan\\IR.c"
...
#line 21 "D:\\avr\\Uart_Interrupt\\LM_Universal\\LM_IAR_V5.0\\Alan\\IR.c"
...
#line 22 "D:\\avr\\Uart_Interrupt\\LM_Universal\\LM_IAR_V5.0\\Alan\\IR.c"
...
#line 23 "D:\\avr\\Uart_Interrupt\\LM_Universal\\LM_IAR_V5.0\\Alan\\IR.c"
...
#line 24 "D:\\avr\\Uart_Interrupt\\LM_Universal\\LM_IAR_V5.0\\Alan\\IR.c"
...
#line 25 "D:\\avr\\Uart_Interrupt\\LM_Universal\\LM_IAR_V5.0\\Alan\\IR.c"
...
__interrupt void usart1_rx_ser(void)
{
  ...
  temp = UDR1;
    ...
    if(IR_reception.begin_actual >= 200)
      IR_reception.begin_actual = 0;
...
__interrupt void usart_tx_ser(void)
{
    if (tx1_counter)
    {
           ...
           if (++tx1_rd_index == 14) tx1_rd_index=0;
    };
}
...
void put_c1(char c)
{
    while (tx1_counter == 14);   //发送数据队列中还有数据没有发送完，等待

    __disable_interrupt();
    if (tx1_counter || ((UCSR1A & (1<<5))==0))    //若发送数据队列有数据或者数据寄存器UDR非空时执行(因为队列先进先出的原因，所以，c要放进非空的发送数据队列里面)
		{
           tx1_buffer[tx1_wr_index]=c;
           if (++tx1_wr_index == 14) tx1_wr_index=0;
           ++tx1_counter;
       }
    else
           UDR1=c;
...
{
	UCSR1A=(1<<7)|(1<<6);
	UCSR1B=(1<<7)|(1<<6)|(1<<4)|(1<<3);		//0xD8
	UCSR1C=(1<<3)|(1<<2)|(1<<1);				//0x06;
	UBRR1H=0x00;
...
{
	TCCR1A = (1<<7)|(0<<6)|(1<<5)|(0<<4)|(0<<1)|(0<<0);
	TCCR1B = (1<<4)|(0<<3)|(0<<2)|(0<<1)|(1<<0);	// CLOCK/1	
}
...
void decode_IR()
{
  ...
  unsigned char group_flag = 0;
	...
	unsigned char current_scenario = 0xFF;
	//CRC checking
      ...
      for(i=15;i!=0xFF;i--)
        ...
        switch(IR_reception.pipe[5])
        {
          case 0x01:    //ON
            ...
            {
              send_aux_frame(1,ns,current_scenario);
            }
            else if(group_flag)
            {
          ...
          case 0x02:    //OFF
            ...
            if(available_command)
            {
                send_aux_frame(2,ns,current_scenario);
            }
            else if(group_flag)
            {
              if(product_information.conf_type == 3)
          ...
          case 0x07:
            ...
            else
            {
              if(ns < 16)
              {
                send_aux_frame(7,ns,current_scenario);
              }
            }
          break;
			...
			case 7:
                ...
                for(i=0;i<16;i++)
                {
                    	...
                    	buffer_scs[11] = buffer_char[1];
											...
											if(avoid_twice == 0)
											{
                    		send_frame_pp_ext_broad(buffer_scs,50);
                ...
                /*
							...
							state.main_state = 0x9018;

						break;

						case 0x07: //close learning
							//strlcpy(buffer_ID,&IR_reception.pipe[2],4);
              if(state.main_state > 0x9015 && state.main_state < 0x9017)
							{
								if(current_id == temp_variable.ID)
								{
									temp_variable.ID = 0;
									state.pnl_slave = 0;
							  	state.main_state = 0x9017;
								}
							}
						break;
                ...
                else if(product_information.conf_type == 2)
                {
                  ...
                  key_result.short_pressed = 1;
						...
						case 0x2D: //NEXT_ACTUATOR
							send_frame_pp_ext_broad("D20001428C000B01",50);
						break;

						case 0x2E: //INCLUDE
							send_frame_pp_ext_broad("D20001428C000B02",50);
						break;

						case 0x2F: //EXCLUDE
							send_frame_pp_ext_broad("D20001428C000B03",50);
						break;

						case 0x30:    //SET_ON
							//SCS_received.Manual_ON = 1;    // add by alan on 20090404  //0715
              send_frame_pp_ext_broad("D20001428C000B04",50);
						break;
						
						case 0x31:    //SET_OFF
							//SCS_received.Manual_OFF = 1;    // add by alan on 20090404  //0715
              send_frame_pp_ext_broad("D20001428C000B05",50);
						break;

            case 0x32:    //SET_DIMMING_UP
              send_frame_pp_ext_broad("D20001428C000B06",50);
            break;

            case 0x33:    //SET_DIMMING_DOWN
              send_frame_pp_ext_broad("D20001428C000B07",50);
            break;
                  ...
                  if(available_command)
                  	...
                  	led_blinking_normal();
						...
						case 0x36: //ADD_PERIPHERAL
							send_frame_pp_ext_broad("D20001428C000802",50);
						break;
              ...
              system_reset();
						...
						case 0x38:			//reset room controler
							send_frame_pp_ext_broad("D20001428C0008F0",50);
						break;
						default:break;
					}	
				}
        ...
        if(IR_reception.pipe[5] == 0x83)  //dimmer step
        {
          if(available_command)
          {
							send_aux_frame(3,ns,current_scenario);
          }
          else if(group_flag)
          {
            if(product_information.conf_type != 3)
        ...
        if(IR_reception.pipe[5] == 0x84)  //go to level time
        {
          if(available_command)
          {
              send_aux_frame(4,ns,current_scenario);
          }
        }
            ...
            else if(par_num == 260)
            {
							...
							(KO_conf . status[0]&=(~(1<<4)));
							(KO_conf . status[0]&=(~(1<<5)));
							(KO_conf . status[0]&=(~(1<<3)));	
            }
						put_s1(buffer_ir,13);
					}
				}
	...
	if(state.main_state == 0x9013)
	{
		...
		else
		{
			if( Time_delay.timer_flex.time_out == 1 )
			{
				...
				state.main_state = 0x9016;
			}
		}
	}

	if(state.main_state == 0x9014)
	{
		if( Time_delay.timer_flex.time_out == 1 )
		{
			led_blinking_disable(0);
